            ...
            <div class="form_container">
					...
					if(mysqli_num_rows($result) > 0)
					{
						while($row = mysqli_fetch_assoc($result))
						{
							$title = $row['title'];
							$content = $row['content'];
							echo "<form action='edit_post.php?pid=$pid' style='width: 100%' class='contact_form' method='post' enctype='multipart/form-data'><input type='text' name='title' value='$title'><br /><br /><textarea name='content' placeholder='Content' rows='20' cols='50'>$content</textarea><button type='submit' name='update'>Update</button></form>";
						}
					}
					?>
					<script>
